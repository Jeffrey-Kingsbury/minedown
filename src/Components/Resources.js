			...
			<Items>
				...
				{Object.keys(RESOURCES.dig).map((resource) => {
					...
					return (
						<p
							key={resource}
							style={{ color: COLOR_PICKER[resource] || 'black' }}
						>
							{resource}:{' '}
							{items[resource] === null
								? 0
								: Number(items[resource]) > 99999999999999
								? Number(items[resource]).toExponential(2)
								: Number(items[resource]).toLocaleString()}
						</p>
					);
				})}
			</Items>
			...
			<Items>
				...
				{Object.keys(RESOURCES.craft).map((resource) => {
					...
					return (
						...
						>
							{resource}:{' '}
							{Number(items[resource]) > 99999999999999 ? Number(items[resource]).toExponential(2) : Number(items[resource]).toLocaleString()}
						</p>
					);
				})}
			</Items>
